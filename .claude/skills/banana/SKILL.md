...
> Installed from [AgriciDaniel/banana-claude](https://github.com/AgriciDaniel/banana-claude) (mirrored as-is, reviewed, no prompt injection found). This skill drives Google's Gemini "Nano Banana" image models through the `@ycse/nanobanana-mcp` MCP server — run `/banana setup` (or `python3 scripts/setup_mcp.py`) first to install and configure that MCP server and a Google AI API key; without it the `gemini_*` tool calls referenced below won't exist.
